...
.. important:: В случае сегментированных кэшей и персистентности третьей стороны, такой как реляционная база данных, не сопоставленные с этим узлом ключи, так же как их основные и резервные копии, автоматически удаляются. Это не относится к **Grid Persistent Store**, где каждый узел хранит только те данные, для которых он является основным или резервным
...
Для случаев, когда необходимо выполнить какую-то пользовательскую логику, а не при добавлении новых данных, можно воспользоваться **API** *StreamReceiver*.

Потоковые приемники позволяют реагировать на потоковые данные в сочетании друг с другом непосредственно на узлах, где они будут кэшироваться. Данные можно изменить или добавить к ним какую-либо пользовательскую логику предварительной обработки прежде, чем помещать их в кэш.

.. important:: *StreamReceiver* не помещает данные в кэш автоматически. Для этого необходимо воспользоваться любым из методов *cache.put(...)*

...
Поставщик отправляет данные непосредственно брокеру, который является лидером партиции, без какого-либо промежуточного уровня маршрутизации. При этом все узлы **ADS** могут ответить на запрос метаданных о том, какие серверы активны, и где лидеры для партиций топика находятся в любой момент времени, чтобы позволить поставщику соответствующим образом направлять свои запросы.

Клиент контролирует, в какую партицию публикуется сообщения. Это может быть сделано произвольно, реализуя некую случайную балансировку нагрузки, или это может быть сделано с помощью некоторой функции семантического разбиения. **ADS** предоставляет интерфейс для семантического разбиения, позволяя пользователю указывать ключ для секционирования и использовать этот хэш в партиции (при необходимости есть возможность переопределить функцию партиции). Например, если выбранный ключ был идентификатором пользователя, тогда все данные для данного пользователя будут отправлены в тот же раздел. Это, в свою очередь, позволит потребителям сделать предположения относительно их потребления. Этот стиль разбиения явно разработан таким образом, чтобы разрешить локально-чувствительную обработку в потребителях.

Клиент контролирует, в какой раздел он публикует сообщения. Это может быть сделано случайным образом, реализуя своего рода случайную балансировку нагрузки, или это может быть сделано с помощью некоторой семантической функции секционирования. Мы предоставляем интерфейс для семантического секционирования, позволяя пользователю указать ключ для секционирования и использовать его для хэширования секции (есть также возможность переопределить функцию секционирования, если это необходимо). Например, если выбран ключ был id пользователя, то все данные для данного пользователя будут отправлены в тот же раздел. Это, в свою очередь, позволит потребителям делать местные предположения о своем потреблении. Этот стиль секционирования явно разработан, чтобы позволить локально-чувствительную обработку в потребителях.
